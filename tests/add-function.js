...
testContactCallback(
  'btDiscreteDynamicsWorld.prototype.setContactAddedCallback should work like a charm',
  (t) => {
    ...
    callback.pointer = Ammo.addFunction(
      callback,
...
testContactCallback(
  'btDiscreteDynamicsWorld.prototype.setContactDestroyedCallback should work like a charm',
  (t) => {
    const world = createDiscreteDynamicsWorld();
    const timestep = 1;
    const maxSubsteps = 0;
    ...
    callback.pointer = Ammo.addFunction(
      callback,
      Ammo.CONTACT_DESTROYED_CALLBACK_SIGNATURE
    );
...
testContactCallback(
  'btDiscreteDynamicsWorld.prototype.setContactProcessedCallback should work like a charm',
  (t) => {
    const world = createDiscreteDynamicsWorld();
    const timestep = 1;
    const maxSubsteps = 0;
    ...
    const callbackPointer = Ammo.addFunction(
      callback,
      Ammo.CONTACT_PROCESSED_CALLBACK_SIGNATURE
    );
...
test.cb(
  'btDynamicsWorld.prototype.setInternalTickCallback should work like a charm',
  (t) => {
    const world = createDiscreteDynamicsWorld();
    ...
    callback.pointer = Ammo.addFunction(
      callback,
      Ammo.INTERNAL_TICK_CALLBACK_SIGNATURE
    );
